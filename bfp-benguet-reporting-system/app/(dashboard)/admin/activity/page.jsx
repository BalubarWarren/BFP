...
export default function ActivityLogPage() {
  ...
  const fetchEntries = async () => {
    try {
      ...
      setError(err.response?.data?.error || 'Failed to load activity log. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to load more activity. Please try again.');
    } finally {
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
        <div className="flex flex-col gap-4 border-b border-gray-200 p-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Loaded 50 at a time, most recent first.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="flex min-h-56 flex-col items-center justify-center bg-gray-50 p-10 text-center">
            <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-sm">
              <AlertTriangle className="w-6 h-6 text-bfp-red" />
            </div>
            <p className="text-lg font-semibold text-bfp-navy">Couldn't load activity</p>
            <p className="mt-1 text-sm text-gray-500">{error}</p>
            <button type="button" onClick={fetchEntries} className="btn btn-secondary mt-4 px-6">
              Try Again
            </button>
          </div>
        ) : entries.length === 0 ? (
          ...
          <>
            ...
            {nextCursor && (
              <div className="border-t border-gray-200 p-4 text-center">
                <button type="button" onClick={loadMore} disabled={loadingMore} className="btn btn-secondary px-6">
